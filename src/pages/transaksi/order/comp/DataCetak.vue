<template>
  ...
  <div class="row content-center q-pt-sm">
    ...
    <div class="content-center col">{{ store.dataCetak?.pelanggan }}</div>
  </div>
  <div class="row content-center q-pb-xs">
    <div class="content-center col-3 text-weight-bold">Alamat</div>
    <div class="content-center col-auto q-pr-sm">:</div>
    <div class="content-center col">{{ store.dataCetak?.alamat }}</div>
  </div>
  <div class="row content-center q-pb-xs">
  ...
  <q-list bordered separator>
    <q-item v-for="item in store.dataCetak?.rincians" :key="item">
      ...
      <q-item-section side top>
        <q-item-label caption>Ket: {{ item.keterangan }}</q-item-label>
        <span class="text-black">{{ formatRpDouble(item.subtotal) }} </span>
      </q-item-section>
    </q-item>
